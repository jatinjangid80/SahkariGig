...
import {
  ...
  ChevronDown, HeartHandshake, Sun, Moon
} from 'lucide-react';
                ...
                {userDropdownOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2.5 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 py-2.5 z-50 text-xs font-semibold animate-in fade-in zoom-in-95 duration-150">
                      ...
                      <button
                        onClick={() => handleNavClick('/dashboard', currentUser?.role === 'Admin' ? 'settings' : 'profile')}
            ...
            <div className="pt-3 border-t border-[#E2E8F0] dark:border-slate-800 space-y-2">
              ...
              {currentUser ? (
                <div className="space-y-2">
                  ...
                  <button
                    onClick={() => handleNavClick('/dashboard', currentUser?.role === 'Admin' ? 'settings' : 'profile')}
                    className="w-full text-left py-2 px-3 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800/60 hover:bg-slate-200 dark:hover:bg-slate-800 flex items-center gap-2 cursor-pointer transition-colors"
                  >
