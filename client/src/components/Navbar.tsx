...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/90 dark:border-slate-800 shadow-2xs transition-colors">
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 pt-2 pb-6 space-y-3 animate-in fade-in duration-200">
          {navLinks.map((link) => (
            <button
              key={link.label}
              type="button"
              onClick={() => handleNavClick(link.path, link.tab)}
              className={`block w-full text-left py-2 text-sm font-semibold transition-colors focus:outline-none cursor-pointer ${
                currentPath === link.path && (!link.tab || workerActiveTab === link.tab)
                  ? 'text-emerald-700 dark:text-emerald-400 font-bold'
